...
router = APIRouter(prefix="/companys", tags=["Companys"])

@router.get("", response_model=list[CompanyViewModel])
async def get_all_companys(async_db: AsyncSession = Depends(get_async_db_context)):
    return await CompanyService.get_companys(async_db)
